...
# APM-Managed Direct MCP Architecture

All MCP servers are defined in `apm.yml` and managed directly by APM. Local
servers run as host stdio processes, while remote services use their native
SSE or HTTP transport. Run `make sync-mcp` to regenerate each agent's config.

The former gateway layer is retired. Do not add separate gateway services,
catalogs, or dynamic `mcp-add` / `mcp-find` dependencies.
