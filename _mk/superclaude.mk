...
install-packages-superclaude: ## SuperClaudeフレームワークをClaude Code向けにインストール
	@BOOL_LINK_CHECK=0; if [ -L "$(CLAUDE_DIR)/CLAUDE.md" ] && [ -e "$(DOTFILES_CLAUDE_DIR)/CLAUDE.md" ] && [ "$$(readlink "$(CLAUDE_DIR)/CLAUDE.md")" = "$(DOTFILES_CLAUDE_DIR)/CLAUDE.md" ]; then BOOL_LINK_CHECK=1; fi; \
	BOOL_SETTINGS_LINK=0; if [ -L "$(CLAUDE_DIR)/settings.json" ] && [ -e "$(DOTFILES_CLAUDE_DIR)/settings.json" ] && [ "$$(readlink "$(CLAUDE_DIR)/settings.json")" = "$(DOTFILES_CLAUDE_DIR)/settings.json" ] && [ -L "$(CLAUDE_DIR)/claude-settings.json" ] && [ "$$(readlink "$(CLAUDE_DIR)/claude-settings.json")" = "$(DOTFILES_CLAUDE_DIR)/settings.json" ]; then BOOL_SETTINGS_LINK=1; fi; \
	...
	@if command -v uv >/dev/null 2>&1 && command -v python3 >/dev/null 2>&1; then \
		if command -v SuperClaude >/dev/null 2>&1; then \
			...
			if [ -x "$(CURDIR)/_scripts/install_superclaude.sh" ]; then \
				bash $(CURDIR)/_scripts/install_superclaude.sh || echo "⚠️  インストールに失敗しましたが続行します"; \
			else \
	...
	@ln -sf "$(DOTFILES_CLAUDE_DIR)/settings.json" "$(CLAUDE_DIR)/claude-settings.json"
	@echo "✅ settings.json & claude-settings.json → $(DOTFILES_CLAUDE_DIR)/settings.json"
	@echo ""
