...
*   **役割**: 利用したい全てのMCPサーバー（GitHub, Playwright, Skillport等）の定義をここに集約します。環境変数（`__HOME__` や `__REPO_ROOT__`等）をプレースホルダとして含めることができます。
...
*   **動作**:
    ...
    3.  stdio 接続のみをサポートするクライアント（例: Cursor）は、`scripts/mcp-sse-proxy.js` を介してこの SSE Gateway に接続します。SSE 対応クライアント（例: Gemini CLI, Antigravity）は SSE Gateway に直接接続します。

### 3. Unified Tool Configuration (統一されたツール設定)
stdio クライアント（例: Cursor）では以下の `proxy` 設定が使われます。
...
直接 SSE をサポートするクライアント（例: Gemini CLI, Antigravity 等）は、直接 `http://localhost:10888/sse` に接続します。
