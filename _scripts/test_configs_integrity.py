...
def get_tracked_files(root, pattern=None):
    ...
    try:
        ...
        if pattern == '*.template':
            return subprocess.run([git_path, 'ls-files', '--', '*.template'], capture_output=True, text=True, check=True, cwd=root).stdout.splitlines()  # nosec B603 B607
        if pattern == '*.jsonc':
            return subprocess.run([git_path, 'ls-files', '--', '*.jsonc'], capture_output=True, text=True, check=True, cwd=root).stdout.splitlines()  # nosec B603 B607
        if pattern is None:
            return subprocess.run([git_path, 'ls-files'], capture_output=True, text=True, check=True, cwd=root).stdout.splitlines()  # nosec B603 B607
