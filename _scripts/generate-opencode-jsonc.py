"""Generate opencode/opencode.jsonc from apm.yml (SSOT) + opencode.jsonc.template.
...
TEMPLATE = REPO_ROOT / "opencode" / "opencode.jsonc.template"
...
# ---------------------------------------------------------------------------
# Provider: extract opencode-specific providers from template
# ---------------------------------------------------------------------------
def _load_template_providers(template_path: Path) -> dict[str, Any]:
    """Extract provider_opencode_specific block from template JSON(C)."""
...
# ---------------------------------------------------------------------------
def build_config(apm: dict[str, Any], template_providers: dict[str, Any]) -> dict[str, Any]:
    cfg: dict[str, Any] = {}
    ...
    for name, prov in (apm.get("provider") or {}).items():
        if name == "sakura":
            ...
            providers[name] = prov
    # Merge opencode-specific providers from template (opencode-go, ollama-cloud, openrouter)
    for name, prov in template_providers.items():
        providers[name] = prov
    if providers:
...
def _serialise(cfg: dict[str, Any]) -> str:
    ...
    lines.append("// Source: apm.yml (SSOT) + opencode.jsonc.template")
...
def main() -> None:
    ...
    template_providers = _load_template_providers(TEMPLATE)

    cfg = build_config(apm, template_providers)
